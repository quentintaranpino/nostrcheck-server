...
const dbInsert = async (tableName :string, fields: string[], values: string[]): Promise<number> =>{
	...
	try{
		...
		logger.debug(JSON.parse(JSON.stringify(dbFileInsert)).insertId + " record inserted into " + tableName + " table")
		conn.end();
...
const initDatabase = async (): Promise<void> => {
	...
	const defaultDomain = await dbSelect("SELECT domain FROM domains WHERE domain = ?", "domain", [config.get('server.host')], registeredTableFields);
	if (defaultDomain == ""){
		logger.warn("Default domain not found, creating it");
		const fields: string[] = ["domain", "active", "comments"];
		const values: string[] = [config.get('server.host'), "1", "Default domain generated by server on first run"];
