...
const initDatabase = async (): Promise<void> => {
	...
	const defaultDomain = await dbSelect("SELECT domain FROM domains WHERE domain = ?", "domain", [app.get('server.host')], registeredTableFields);
	if (defaultDomain == ""){
		logger.warn("Default domain not found, creating it");
		const fields: string[] = ["domain", "active", "comments"];
		const values: string[] = [app.get('server.host'), "1", "Default domain generated by server on first run"];
	...
	if (publicLightning == ""){
		...
		const values: string[] = [app.get('server.pubkey'), "serverLightningAddress", "Public lightning redirect generated by server on first run, edit this to recieve SATS on your wallet"];
