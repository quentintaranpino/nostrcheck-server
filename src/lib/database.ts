...
const initDatabase = async (): Promise<void> => {
	...
	if (publicUsername == "" || publicUsername == undefined || publicUsername == null){
		...
		const createPublicUser = await addNewUsername("public", app.get("config.server")["pubkey"], "", config.get('server.host'), "public username generated by server on first run");
