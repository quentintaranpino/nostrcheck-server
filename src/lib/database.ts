...
const initDatabase = async (): Promise<void> => {
	...
	const dbtables = await populateTables(getConfig(null, ["database", "droptables"]));
	if (!dbtables) {
	...
	if (defaultDomain == ""){
		...
		if (insert === 0){
			logger.fatal(`initDatabase - Error creating default domain (${serverHost}). Exiting.`);
			process.exit(1);
		}
	...
	if (publicUsername == "" || publicUsername == undefined || publicUsername == null){
		...
		const createPublicUser = await addNewUsername("public", serverPubkey, "", serverHost, "public username generated by server on first run", true);

		if (createPublicUser == 0){
			logger.fatal(`initDatabase - Error creating public username (${serverPubkey}). Exiting.`);
			process.exit(1);
		}

		const allowed = await dbUpdate("registered",{"allowed" : "1" }, ["username"], ["public"]);
		if (!allowed) {
			logger.fatal(`initDatabase - Error updating (allowed -> 1) public username (${serverPubkey}). Exiting.`);
			process.exit(1);
		}
	}
	...
	if (publicLightning == ""){
		...
		if (insert === 0){
			logger.fatal(`initDatabase - Error creating public lightning address (${serverPubkey}). Exiting.`);
			process.exit(1);
		}
