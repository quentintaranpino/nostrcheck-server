...
export const loadAdminEndpoint = async (app: Application, version:string): Promise<void> => {
        ...
        if (version == "v2"){
                ...
                // Moderate DB record
                app.post("/api/" + version + app.get("config.server")["availableModules"]["admin"]["path"] + "/moderaterecord", moderateDBRecord);
                ...
                app.post("/api/" + version + app.get("config.server")["availableModules"]["admin"]["path"] + "/updatetheme/", updateTheme);
