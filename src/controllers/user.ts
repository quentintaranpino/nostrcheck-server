...
const setUserPrefs = async (req: Request, res: Response): Promise<Response> => {
    ...
    if (!isModuleEnabled("frontend", app)) {
        logger.info("Attempt to access a non-active module:","frontend","|","IP:", reqInfo.ip);
        return res.status(403).send({"status": "error", "message": "Module is not enabled"});
    }

    logger.info("REQ -> setUserPrefs", req.hostname, "|", reqInfo.ip);
...
const getUserPrefs = async (req: Request, res: Response): Promise<Response> => {
    ...
    logger.info("REQ -> getUserPrefs", req.hostname, "|", reqInfo.ip);
    res.setHeader('Content-Type', 'application/json');
